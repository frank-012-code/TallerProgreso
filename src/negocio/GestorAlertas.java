...
public class GestorAlertas {
    ...
    // Cambiado a String para poder capturar la alerta desde el Main e Historial
    public String evaluarEstadoComponente(String nombreVehiculo, String nombreComponente, double vidaUtilBaseKm, Caracteristicas operacion) {
        ...
        String alertaGenerada = "";

        if (porcentajeDesgaste >= 100) {
            alertaGenerada = "CRÍTICO: El componente '" + nombreComponente + "' del " + nombreVehiculo + " ha superado su vida útil límite (" + kmActuales + " km). Reemplazo inmediato.";
            registrarAlerta(alertaGenerada);
        } else if (porcentajeDesgaste >= 85) {
            alertaGenerada = "ADVERTENCIA PREVENTIVA: El componente '" + nombreComponente + "' del " + nombreVehiculo + " está al " + String.format("%.2f", porcentajeDesgaste) + "% de desgaste. Proyectar repuesto en stock.";
            registrarAlerta(alertaGenerada);
        } else {
